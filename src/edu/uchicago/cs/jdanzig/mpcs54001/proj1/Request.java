...
public class Request {
	...
	private static Pattern requestLinePattern = Pattern.compile("^([A-Z]+)\\s([^\\s]+)\\sHTTP\\/([0-9]\\.[0-9])$");
	private static Pattern headerLinePattern = Pattern.compile("^([^\\s:]+):\\s*(.*)\\s*$");
	private static Pattern safePathPattern = Pattern.compile("^(\\/(?:[A-Za-z0-9]|(?<!\\.)\\.|(?<!\\/)\\/)*)$");
	...
	public Request(String requestLine) throws HTTPErrorException {
		...
		try {
			path = m.group(2);
			// If the path doesn't match this pattern, it could be a huge security hole
